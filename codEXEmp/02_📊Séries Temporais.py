...
def get_daily_precip(collection, roi, start_year, end_year):
    """
    Substitui o uso de getRegion por um map que reduz cada imagem sobre a ROI,
    retornando uma linha por data (evita exceder o limite de pontos do getRegion).
    """
    def daily_sum(img):
        # Reduz a imagem na ROI (uma única estatística por imagem)
        total = img.reduceRegion(
            reducer=ee.Reducer.sum(),
            geometry=roi,
            scale=SCALE,
